...
import { ArrowDown, BarChart2, ChevronDown, TrendingUp as ProfitIcon, PieChart as PieChartIcon, ChevronUp, IndianRupee, PiggyBank, Plus, ShoppingCart, TrendingDown, TrendingUp, Wallet } from 'lucide-react';
...
const Dashboard = () => {
  ...
  //function to check ifa date is within range
  ...
  const displayedExpense = showAllExpense 
    ...
    const fetchDashboardOverview=async () => {
      try {
        setLoading(true);
        const res=await axios.get(`${server}/api/dashboard`,{
    ...
    const handleAddTransaction=async () => {
      ...
      try {
        ...
        if(newTransaction.type==="income"){
          await axios.post(`${server}/api/income/add`,payload,{
            headers:getAuthHeader(),
          });
        }else{
          await axios.post(`${server}/api/expense/add`,payload,{
            headers:getAuthHeader(),
          });
        } 
    ...
    <div className={dashboardStyles.container}>
      ...
      <div className={dashboardStyles.summaryGrid}>
        <FinancialCard icon={
          ...
          } label="Total Balance" value={`Rs${Math.round(displayIncome-displayExpenses).toLocaleString()}`}
          additionalContent={
            <div className='flex items-center gap-2 mt-2 text-sm'>
              <span className={dashboardStyles.balanceBadge}>
                +Rs{Math.round(displayIncome).toLocaleString()}
              </span>
              <span className={dashboardStyles.expenseBadge}>
                -Rs{Math.round(displayExpenses).toLocaleString()}
              </span>
            </div>
          }
        />
        ...
        <FinancialCard icon={
          ...
          value={`Rs${Math.round(displaySavings).toLocaleString()}`}
          additionalContent={
        ...
        <FinancialCard icon={
          ...
          value={`Rs${Math.round(displayExpenses).toLocaleString()}`}
          additionalContent={
      ...
      <div className={dashboardStyles.gaugeGrid}>
        {gaugeData.map((gauge)=>(
          <GaugeCard key={gauge.name}
            gauge={gauge}
            colotInfo={GAUGE_COLORS[gauge.name]}
