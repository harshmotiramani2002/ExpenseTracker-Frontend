...
const Profile = ({user: initialUser, onUpdateProfile, onLogout }) => {
  ...
  const handleSaveProfile=async () => {
    try {
        ...
        toast.error(err.response?.data?.message || "Failed to update profilr");
    }
  };
  ...
  const handlePasswordSubmit=async (e) => {
    ...
    try {
        ...
        toast.error(error.response?.data?.message || "Failed to change profilr");
    }
  };
  ...
  const closePasswordmodal=useCallback(()=>{
    if(!loading){
        ...
        setPasswordErrors({});
        //reset password visibility
        setShowPassword({curr:false,new:false,confirm:false});
    }
  },[loading]);
