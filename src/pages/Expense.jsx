...
const ExpensePage = () => {
  ...
  const fetchOverview = useCallback(async (range = timeFrame ?? "monthly") => {
    try {
      const res = await axios.get(`${server}/api/expense/overview`, {
  ...
  const handleExport = async () => {
    try {
      const res = await axios.get(`${server}/api/expense/downloadexcel`, {
        headers: getAuthHeaders(),
        responseType: "blob",
      });
        ...
        <div className={styles.chartHeight}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
              ...
              />
              <Tooltip
                formatter={(value) => [`Rs ${Math.round(value).toLocaleString()}`, "Expense"]}
