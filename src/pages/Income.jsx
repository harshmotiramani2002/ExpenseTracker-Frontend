...
const Income = () => {
  ...
  const fetchOverview = useCallback(
    async (range = timeFrame ?? "monthly") => {
      try {
        const res = await axios.get(`${server}/api/income/overview`, {
  ...
  const handleEditTransaction = useCallback(async () => {
    ...
    try {
      ...
      await axios.put(`${server}/api/income/update/${editingId}`, payload, {
        headers: { "Content-Type": "application/json", ...getAuthHeaders() },
      });
  ...
  const handleDeleteTransaction = useCallback(
    async (id) => {
      ...
      try {
        setLoading(true);
        await axios.delete(`${server}/api/income/delete/${id}`, {
          headers: getAuthHeaders(),
        });
  ...
  const handleExport = useCallback(async () => {
    try {
      const res = await axios.get(`${server}/api/income/downloadexcel`, {
        headers: getAuthHeaders(),
        responseType: "blob",
      });
