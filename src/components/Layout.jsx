...
import { ArrowDown, ArrowUp, Car, ChevronDown, ChevronUp, Clock, CreditCard, Gift, Home, IndianRupee, Info, PieChart, PiggyBank, RefreshCw, ShoppingCart, TrendingUp, Utensils, Zap } from 'lucide-react';
...
const Layout = ({onLogout,user}) => {
  ...
  //fetch the transaction from server
  const fetchTransactions = async () => {
    try {
      ...
      const [incomeRes, expenseRes] = await Promise.all([
        axios.get(`${server}/api/income/get`, { headers }),
        axios.get(`${server}/api/expense/get`, { headers }),
      ]);
  ...
  const addTransaction = async (transaction) => {
    try {
      ...
      await axios.post(`${server}/api/${endpoint}`, transaction, { headers });
  ...
  const editTransaction = async (id, transaction) => {
    try {
      ...
      await axios.put(`${server}/api/${endpoint}/${id}`, transaction, {
        headers,
      });
      await fetchTransactions();
      return true;
  ...
  const deleteTransaction = async (id, type) => {
    try {
      ...
      await axios.delete(`${server}/api/${endpoint}/${id}`, { headers });
      await fetchTransactions();
      return true;
    } catch (err) {
      console.error(
  ...
  //get the stats data according to time
  const stats = useMemo(() => {
    const now = new Date();
            ...
            <div className={styles.statCards.card}>
              <div className={styles.statCards.cardHeader}>
                <div>
                  ...
                  <p className={styles.statCards.cardValue}>
                    Rs. {stats.allTimeSavings.toLocaleString("en-IN", {
              ...
              <p className={styles.statCards.cardFooter}>
                <span className='text-teal-600 font-medium'>
                  +Rs{stats.last30DaysSavings.toLocaleString()}
                </span>{" "}
                This Month
              </p>
            </div>
            ...
            <div className={styles.statCards.card}>
              <div className={styles.statCards.cardHeader}>
                <div>
                  ...
                  <p className={styles.statCards.cardValue}>
                    Rs. {stats.last30DaysIncome.toLocaleString("en-IN", {
                      maximumFractionDigits:2,
                    })}
                  </p>
                </div>
            ...
            <div className={styles.statCards.card}>
              <div className={styles.statCards.cardHeader}>
                <div>
                  ...
                  <p className={styles.statCards.cardValue}>
                    Rs. {stats.last30DaysExpenses.toLocaleString("en-IN", {
                      maximumFractionDigits:2,
                    })}
                  </p>
                </div>
              ...
              <div className={styles.cards.base}>
                <div className={styles.transactions.cardHeader}>
                  <h3 className={styles.transactions.cardTitle}>
                    <Clock className='2-6 h-6 text-purple-500'/>
                    Recent Transactions
                  </h3>
                ...
                <div className={styles.transactions.dataStackingInfo}>
                  ...
                  <span>Transactions are stacked by date (newwst first)</span>
                </div>
